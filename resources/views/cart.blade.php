            ...
            <label class="weui_cell weui_check_label" for="{{$cart_item->product->product_id}}">
                ...
                <div class="weui_cell_bd weui_cell_primary">
                    <div style="position: relative;">
                        <img class="bk_preview" src="{{$cart_item->product->preview}}" class="m3_preview" onclick="_toProduct({{$cart_item->product->product_id}});"/>
...
@section('my-js')
    <script>
        ...
        function _onDelete(){
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.status != 0){
                        $('.bk_toptips').show();
                        $('.bk_toptips span').html(data.content);
                        setTimeout(function () {
                            $('.bk_toptips').hide();
                        },2000);
                        return;
        ...
        function _toProduct(product_id) {
            // alert(product_id);
