...
class ValidateController extends Controller {
    ...
    public function sendSMS(Request $request){
        ...
        $code = '';
        $chrset = '1234567890';
        $_len = strlen($chrset) -1;
        for($i = 0; $i < 6; ++$i){
            $code .= $chrset[mt_rand(0,$_len)];
        }
        $m3_result = $sendTemplateSMS->sendTemplateSMS($phone, array($code, 60), 1);
        if($m3_result->status == 0){
            $temPhone = TempPhone::where('phone', $phone)->first();
            if($temPhone == null){
                $temPhone = new TempPhone;
            }
            $temPhone->phone = $phone;
            $temPhone->code = $code;
            $temPhone->deadline = date('Y-m-d H-i-s', time() + 60*60);
            $temPhone->save();
        }
        return $m3_result->toJson();
    }
